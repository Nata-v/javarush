...
public class Snake {
    ...
    //метод перемещаеи змею на один ход. Направление перемещения задано переменной direction
    public void move(){
        if (!isAlive()){
            return;
    ...
    // метод перемещает змею в соседнюю клетку. координаты клетки заданы относительно текущей головы с помощью
    // переменных (dx, dy).
